        ...
        public void ProcessOrder(Cart cart, ShippingDetails shippingInfo)
        {
            using (var smtpClient = new SmtpClient())
            {
                ...
                foreach (var line in cart.Lines)
                {
                    ...
                    body.AppendFormat("{0} x {1} (Всього: {2:c}",
                        line.Quantity, line.Game.Name, subtotal);
                }

                body.AppendFormat("Загальна вартiсть: {0:c}", cart.ComputeTotalValue())
                    .AppendLine("---")
                    .AppendLine("Доставка:")
                    ...
                    .AppendFormat("Подарункова упаковка: {0}",
                        shippingInfo.GiftWrap ? "Так" : "Нi");

                MailMessage mailMessage = new MailMessage(
                                       emailSettings.MailFromAddress,	// От кого
                                       emailSettings.MailToAddress,		// Кому
                                       "Нове замовлення вiдправлено!",		// Тема
